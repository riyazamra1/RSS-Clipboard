...
class MainActivity : ComponentActivity() {
    ...
    private fun startFloatingService() {
        if (Build.VERSION.SDK_INT >= 33 && FloatingPrefs.notifications(this) && checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) notificationPermission.launch(Manifest.permission.POST_NOTIFICATIONS)
...
@Composable private fun SettingsScreen(onOpenFloating:()->Unit,onOpenTheme:()->Unit,onBatteryOptimization:()->Unit){
    ...
    Scaffold(topBar={TopAppBar(title={Text("Settings")})}){pad->Column(Modifier.fillMaxSize().padding(pad).padding(12.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
        ...
        SettingsRow(Icons.Default.Notifications,"Notifications", if (FloatingPrefs.notifications(context)) "On — low-power foreground status notification" else "Off — notification visibility reduced",{
            val next=!FloatingPrefs.notifications(context); FloatingPrefs.setNotifications(context,next); if(Build.VERSION.SDK_INT>=26){val nm=context.getSystemService(android.app.NotificationManager::class.java);nm.deleteNotificationChannel("rss_clipboard_monitor");nm.createNotificationChannel(android.app.NotificationChannel("rss_clipboard_monitor","Clipboard monitor",if(next)android.app.NotificationManager.IMPORTANCE_LOW else android.app.NotificationManager.IMPORTANCE_NONE))}
        })
